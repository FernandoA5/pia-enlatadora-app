...
interface ClienteLike extends Partial<ClienteFormValues> {
...
const form = reactive<ClienteFormValues>({
  nombre: '',
  telefono: '',
  direccion: '',
  correo: ''
...
const modalTitle = computed(() => (props.cliente?.id ? 'Editar cliente' : 'Registrar cliente'))
const submitLabel = computed(() => (props.cliente?.id ? 'Actualizar' : 'Registrar'))
...
const validate = () => {
  ...
  if (!form.direccion.trim()) {
    errors.direccion = 'Ingresa la dirección del cliente'
  }
...
const handleSubmit = () => {
  ...
  emit('submit', { data: payload, id: props.cliente?.id ?? null })
}
