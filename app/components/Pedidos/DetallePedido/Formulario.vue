<template>
  <GenericModal
    ...
    content-class="detalle-pedido-modal-content"
    @close="handleClose"
  >
    <form class="modal-content" @submit.prevent="handleSubmit">
      <div class="form-row">
        <label class="form-field form-field--producto">
          <span>Producto</span>
          <select v-model="form.id_producto" :disabled="loading">
            <option value="">Selecciona un producto</option>
            <option
              v-for="option in productos"
              :key="option.value"
          ...
          <p v-if="errors.id_producto" class="field-error">{{ errors.id_producto }}</p>
        </label>
...
type DetallePedidoLike = Partial<DetallePedidoForm> & {
  id?: number | string | null
  id_pedido?: number | string | null
}

export type DetallePedidoGuardado = {
  id?: number | string | null
  id_pedido?: number | string | null
...
const props = withDefaults(
  defineProps<{
    modelValue: boolean
    detalle?: DetallePedidoLike | null
    loading?: boolean
    productos?: OptionValue[]
    idPedido: string | number
    pedido?: GenericTableItem | null
    detallesGuardados?: DetallePedidoGuardado[]
  }>(),
  {
    detalle: null,
    loading: false,
...
const form = reactive<DetallePedidoForm>({
  cantidad: '',
  precio_unitario: '',
  id_producto: ''
})
...
const detalleHeaders: GenericTableHeader[] = [
  { key: 'producto', title: 'Producto', align: 'start' },
...
const { updateDetallePedido, deactivateDetallePedido } = usePedidosApi()
...
const canAgregarDetalle = computed(() => {
  ...
  if (!toStringValue(form.id_producto).trim()) {
    return false
  }
...
const modalTitle = computed(() => (props.detalle?.id ? 'Editar detalle de pedido' : 'Registrar detalle de pedido'))
...
const parseIdProducto = (value: string): string | number => {
...
const findProductoLabel = (value: string) => {
  const option = props.productos?.find(option => String(option.value) === String(value))
  return option?.label ?? 'Producto sin nombre'
}
...
const createDetalleRow = (params: {
  ...
  return {
    ...
    producto: params.productoNombre ?? findProductoLabel(params.idProducto),
...
const syncDetallesGuardados = () => {
  ...
  registros.forEach(registro => {
    const idProducto = toStringValue(registro.id_producto ?? '')
    if (!idProducto) {
      return
    }
    ...
    const row = createDetalleRow({
      ...
      cantidad,
      precioUnitario,
      detalleId: registro.id ?? registro.id_pedido ?? null,
      productoNombre: registro.producto ?? null,
      persisted: true,
      updated: false
    })
...
const agregarDetalle = async () => {
  ...
  const idProducto = form.id_producto
...
const eliminarDetalle = async (uid: string) => {
  ...
  if (detalle.persisted && detalle.detalleId != null) {
    try {
      await deactivateDetallePedido(detalle.detalleId, {
        cantidad: detalle.cantidad,
        precio_unitario: detalle.precio_unitario
      })
      emit('updated')
    } catch (error) {
      console.error('Error al desactivar detalle de pedido:', error)
...
watch(
  ...
  () => {
    const actualizados: DetalleRow[] = detalles.value.map(detalle => ({
      ...detalle,
      producto: findProductoLabel(detalle.id_producto)
    }))
...
:deep(.detalle-pedido-modal-content) {
  display: flex;
  flex-direction: column;
  flex: 1;
  height: 100%;
